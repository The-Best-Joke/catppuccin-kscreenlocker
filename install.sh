...
# [Greeter]Theme= key in kscreenlockerrc is not consulted at lock time.
# See CONTRIBUTING.md ("How install actually works") for context.
#
...
prompt_choice() {
    ...
    local i
    for i in "${!choices[@]}"; do
        [[ "${choices[$i]}" == "$default" ]] && default_idx=$((i+1))
    done
    ...
    for i in "${!choices[@]}"; do
        ...
        printf '%s %2d) %s\n' "$marker" $((i+1)) "${choices[$i]}" >&2
    done

    local input choice
    while true; do
        printf '\nSelect by number or name [%d=%s]: ' "$default_idx" "$default" >&2
        read -r input
        ...
        elif contains "$input" "${choices[@]}"; then
            printf '%s' "$input"
            return
        fi
...
// SPDX-License-Identifier: GPL-2.0-or-later
//
// Auto-generated by install.sh for flavor=${flavor}, accent=${accent}.
// Do not edit; re-run \`./install.sh --apply\` to regenerate.
